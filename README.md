...
Серверы http.net и https.net считают, что входной поток идет в кодировке по умолчанию UTF-8, кроме случая, если в заголовке "Content-Type" указан параметр charset с другой кодировкой. В случае prg скриптов серверы учитывают кодировку, указанную в настройках VFP и оперирует текстом, размещенным или размещаемым в стандартном вводе-выводе VFP именно в этой кодировке. Дополнительно самому prg скрипту ничего перекодировать не требуется. В качастве стандартного ввода-вывода для VFP серверы формируют объект _Screen.STD_IO на базе типа EditBox. Как эффективно читать и писать в этот объект приведено в примере выше. По окончанию работы prg скрипта объект _Screen.STD_IO удаляется. Не смотря на то, что согласно документации в объектах типа EditBox можно размещать текст объемом до 2 гигабайт, мне удалось разместить в нем до критической ошибки о нехватке памяти около 200 мегабайт.  
