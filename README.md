...
Серверы http.net и https.net считают кодировкой по умолчанию UTF-8. Если вы используете UTF-8, то в заголовке запросов "Content-Type" кодировку UTF-8 обычно не указывают. Если в заголовке указана другая кодировка, то она будет использована. В случае prg скриптов серверы учитывают кодировку, указанную в настройках VFP и оперирует текстом, размещенным или размещаемым в стандартном вводе-выводе именно в этой кодировке. Дополнительно самому prg скрипту ничего перекодировать не требуется. В качастве стандартного ввода-вывода для VFP серверы формируют объект _Screen.STD_IO на базе типа EditBox. Как эффективно читать и писать в этот объект приведено в примере выше. По окончанию работы prg скрипта объект _Screen.STD_IO удаляется. Не смотря на то, что согласно документации в объектах типа EditBox можно размещать текст объемом до 2 гигабайт, мне удалось разместить в нем до критической ошибки о нехватке памяти около 200 мегабайт.  
